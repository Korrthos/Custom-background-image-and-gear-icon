...
from .config import addon_path, user_path, addonfoldername, localfilesname, gc


def add_bg_img(imgname, location, review=False):
    #add background image for normal and nightmode
    img_web_rel_path  = f"/_addons/{localfilesname}/user_files/background/{imgname}"
    if location == "body":
...
def get_bg_img():
    bg_abs_path = os.path.join(user_path, "user_files", "background")
